...
由于鸣潮可以后台运行，为了追求最高效率可以用该模板创建两个实例，一个实例单独启用鸣潮并设置后台运行，另一个实例启用其他任务并设置前台运行。然而由于BetterGI和ZZZ-OneDragon肘不赢ok-ww，在前后台同时运行时会被鸣潮抢前台窗口，以下方法只能配合[HonkaiHelper](https://github.com/Aues6uen11Z/HonkaiHelper)使用，让崩三和鸣潮同时前后台跑，其他前台任务放崩三后面：
